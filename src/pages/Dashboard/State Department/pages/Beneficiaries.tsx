...
const Beneficiaries = () => {
  ...
  }, [beneficiaries, searchTerm, filterStatus, filterDistrict, filterTribe]);
  ...
  const totalFamilyMembers = beneficiaries.reduce((sum, b) => sum + b.familyMembers, 0);
  const avgIncome = beneficiaries.length ? (beneficiaries.reduce((sum, b) => sum + b.monthlyIncome, 0) / beneficiaries.length) : 0;
  ...
  return (
    <div className="space-y-6 p-6 max-w-7xl mx-auto bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-2">
        <div>
          <h1 className="text-3xl font-semibold text-slate-900">Patta Holders & Beneficiaries</h1>
          <p className="text-sm text-slate-600">Forest Rights Act Beneficiaries - Madhya Pradesh</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold">{beneficiaries.filter(b => b.status === 'Active').length}</p>
          </div>
