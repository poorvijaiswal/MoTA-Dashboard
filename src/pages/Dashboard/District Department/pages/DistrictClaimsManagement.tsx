...
const ClaimsManagement = () => {
  ...
  const [districtFilter] = useState('Indore'); // district fixed to Indore
  const [villageFilter, setVillageFilter] = useState('all'); // new village filter state
  ...
  // filter claims for Indore district only
  const indoreClaims = useMemo(() => {
    return claimsData.claims.filter(claim => claim.state === 'Madhya Pradesh' && claim.district === 'Indore');
  }, []);

  // Unique villages in Indore for village filter dropdown
  const indoreVillages = useMemo(() => {
    return [...new Set(indoreClaims.map(c => c.village))].sort();
  }, [indoreClaims]);
  ...
  const filteredClaims = useMemo(() => {
    return indoreClaims.filter(claim => {
        ...
        return matchesSearch && matchesStatus && matchesVillage && matchesType;
      })
      .sort((a, b) => {
        let aValue = a[sortBy];
        let bValue = b[sortBy];
  ...
  }, [indoreClaims, searchTerm, statusFilter, villageFilter, typeFilter, sortBy, sortOrder]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage Forest Rights Act claims - Indore District Only</p>
        </div>
      ...
      <Card className="shadow-card">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            {/* District filter fixed to Indore */}
            <Select value="Indore" disabled>
              <SelectTrigger>
      ...
      <Card className="shadow-card">
        <CardHeader>
          ...
          <CardDescription>
            Showing {currentClaims.length} of {totalClaims} claims for Indore District
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <TableRow key={claim.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-1 text-sm">
                        ...
                        <span>{claim.village}, Indore</span>
                      </div>
                      <div className="text-xs text-muted-foreground">Madhya Pradesh</div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-1 text-sm">
                      ...
                      {claim.approval_date && (
                        <div className="text-xs text-success">Approved: {claim.approval_date}</div>
                      )}
                    </TableCell>
                    <TableCell>
                      <div className="flex space-x-2">
                        <Button size="sm" variant="outline" onClick={() => navigate(`/profile/${claim.id}`)}>View</Button>
                        {claim.status === 'pending' && (
                          <Button size="sm" variant="default" className="bg-[#1B365D] hover:bg-[#0F1B2E]">Process</Button>
                        )}
                      </div>
                    </TableCell>
                  </TableRow>
