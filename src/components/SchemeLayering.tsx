...
const SchemeLayering = () => (
  <div className="p-6 bg-gray-50 min-h-screen">
    ...
    <div className="mb-4 text-gray-700">
      <span className="font-semibold">See which schemes are linked and what interventions are recommended for each patta holder.</span>
    </div>
    <div className="overflow-x-auto rounded shadow">
      <table className="w-full text-sm bg-white">
        ...
        <tbody>
          {holders.map((h, idx) => (
            <tr key={h.id} className={idx % 2 === 0 ? "bg-white hover:bg-green-50 transition" : "bg-green-50 hover:bg-green-100 transition"}>
              <td className="py-2 px-2 font-semibold">{h.name}</td>
              <td className="py-2 px-2">
                {h.schemes_linked.map(scheme => (
                  <span key={scheme} className="inline-block bg-green-200 text-green-900 rounded px-2 py-1 mr-1 mb-1 text-xs">
                    {scheme}
                  </span>
                ))}
              </td>
